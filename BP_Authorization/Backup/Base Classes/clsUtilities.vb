        ...
        Try
            ...
            If oDoc.Browser.GetByKeys(strObjectKey) Then
                ...
                For docLineindex As Integer = 0 To oDoc_Lines.Count - 1
                    ...
                    If oDoc_Lines.UserFields.Fields.Item("U_JEDocEty").Value.ToString.Length = 0 Then
...
#Region "Update Costing "
            ...
            If oDoc.Browser.GetByKeys(strObjectKey) Then
                ...
                For docLineindex As Integer = 0 To oDoc_Lines.Count - 1
                    ...
                    If oProduction.GetByKey(oDoc_Lines.BaseEntry) Then
                        ...
                        'Update of Production Item Costs...
                        ...
                        For intWhsIndex As Integer = 0 To oItemWhs.Count - 1
                            ...
                            If oItemWhs.WarehouseCode = oBOM_Lines.Warehouse Then
                                If (oItem.ItemType = SAPbobsCOM.ItemTypeEnum.itItems) Then
                                    ...
                                    Else
                                        dblRMCost += CDbl(oBOM_Lines.Quantity) * CDbl(oItemWhs.StandardAveragePrice)

                                        'dblFixCost += CDbl(oBOM_Lines.Quantity) * CDbl(oItemWhs.UserFields.Fields.Item("U_AvgFLbCst").Value)
                                        'dblVarCost += CDbl(oBOM_Lines.Quantity) * CDbl(oItemWhs.UserFields.Fields.Item("U_AvgVLbCst").Value)
                                    End If
                                ElseIf (oItem.ItemType = SAPbobsCOM.ItemTypeEnum.itLabor) Then
                                    If oItem.UserFields.Fields.Item("U_LabType").Value = "F" Then
                                        dblFixCost += CDbl(oBOM_Lines.Quantity) * CDbl(oItemWhs.StandardAveragePrice)
                                    Else
                                        dblVarCost += CDbl(oBOM_Lines.Quantity) * CDbl(oItemWhs.StandardAveragePrice)
                                    End If
                                End If
                            End If
                        Next
    ...
    Public Function update_RMPCosting(ByVal oForm As SAPbouiCOM.Form, ByVal strDocType As String, ByVal strObjectKey As String)
        Dim _retVal As Boolean = True
        Try
            Dim strQuery As String
            Dim oDoc As SAPbobsCOM.Documents = Nothing
                    ...
                    If oDocItem.GetByKey(oDoc_Lines.ItemCode) Then
                        ...
                        For intWhsIndex As Integer = 0 To oDocWhs.Count - 1
                            ...
                            If oDocWhs.WarehouseCode = oDoc_Lines.WarehouseCode Then
                                ...
                                'Dim dblARMCost As Double = CDbl(((dblICost * oDoc_Lines.Quantity) + (dblAvgRMCost * (oDocWhs.InStock - oDoc_Lines.Quantity))) / oDocWhs.InStock)
                                'Dim dblAFLCost As Double = CDbl(((0 * oDoc_Lines.Quantity) + (dblAvgFLbCost * (oDocWhs.InStock - oDoc_Lines.Quantity))) / oDocWhs.InStock)
                                'Dim dblAVLCost As Double = CDbl(((0 * oDoc_Lines.Quantity) + (dblAgvVLbCost * (oDocWhs.InStock - oDoc_Lines.Quantity))) / oDocWhs.InStock)
                            ...
                            If oDocWhs.WarehouseCode = oDoc.FromWarehouse Then
                                ...
                                For intWhsToWhs As Integer = 0 To oDocWhs.Count - 1
                                    ...
                                    If oDocWhs.WarehouseCode = oDoc_Lines.WarehouseCode Then
                                        ...
                                        oDocWhs.UserFields.Fields.Item("U_AvgRMCst").Value = CDbl(((dblTCost * oDoc_Lines.Quantity) + (dblAvgRMCost * (oDocWhs.InStock - oDoc_Lines.Quantity))) / oDocWhs.InStock)
                                        oDocWhs.UserFields.Fields.Item("U_AvgFLbCst").Value = CDbl(((0 * oDoc_Lines.Quantity) + (dblAvgFLbCost * (oDocWhs.InStock - oDoc_Lines.Quantity))) / oDocWhs.InStock)
                                        oDocWhs.UserFields.Fields.Item("U_AvgVLbCst").Value = CDbl(((0 * oDoc_Lines.Quantity) + (dblAgvVLbCost * (oDocWhs.InStock - oDoc_Lines.Quantity))) / oDocWhs.InStock)
